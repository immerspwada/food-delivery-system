...
('นายทดสอบ ระบบ', '083-456-7890', '789 ถนนรัตนาธิเบศรแขวงบางกระสอ เขตบางพลัด กรุงเทพฯ 10700');

-- Insert sample orders (you might want to adjust the UUIDs or let them auto-generate)
-- Note: In production, you would typically insert orders through the API
...
BEGIN
    -- Get customer ID
    SELECT id INTO customer_uuid FROM customers WHERE name = 'สมชาย ใจดี' LIMIT 1;
    
    -- Get menu item IDs
    SELECT id INTO menu1_uuid FROM menu_items WHERE name = 'ข้าวผัดกุ้ง' LIMIT 1;
    SELECT id INTO menu2_uuid FROM menu_items WHERE name = 'น้ำมะนาวโซดา' LIMIT 1;
    
    -- Insert order
    INSERT INTO orders (customer_id, customer_name, customer_phone, customer_address, status, total_amount, notes)
    VALUES (customer_uuid, 'สมชาย ใจดี', '081-234-5678', '123 ถนนสุขุมวิท แขวงคลองเตย เขตคลองเตย กรุงเทพฯ 10110', 'delivered', 124.00, 'ไม่ใส่ผัก')
    RETURNING id INTO order_uuid;
    
    -- Insert order items
    INSERT INTO order_items (order_id, menu_item_id, menu_item_name, quantity, unit_price, total_price)
    VALUES 
    (order_uuid, menu1_uuid, 'ข้าวผัดกุ้ง', 1, 89.00, 89.00),
    (order_uuid, menu2_uuid, 'น้ำมะนาวโซดา', 1, 35.00, 35.00);
END $$;
...
BEGIN
    -- Get customer ID
    SELECT id INTO customer_uuid FROM customers WHERE name = 'สมหญิง รักดี' LIMIT 1;
    
    -- Get menu item IDs
    SELECT id INTO menu1_uuid FROM menu_items WHERE name = 'ผัดไทยกุ้งสด' LIMIT 1;
    SELECT id INTO menu2_uuid FROM menu_items WHERE name = 'ชาไทย' LIMIT 1;
    
    -- Insert order
    INSERT INTO orders (customer_id, customer_name, customer_phone, customer_address, status, total_amount, notes)
    VALUES (customer_uuid, 'สมหญิง รักดี', '082-345-6789', '456 ถนนพหลโยธิน แขวงจตุจักร เขตจตุจักร กรุงเทพฯ 10900', 'pending', 120.00, 'เผ็ดน้อย')
    RETURNING id INTO order_uuid;
    
    -- Insert order items
    INSERT INTO order_items (order_id, menu_item_id, menu_item_name, quantity, unit_price, total_price)
    VALUES 
    (order_uuid, menu1_uuid, 'ผัดไทยกุ้งสด', 1, 95.00, 95.00),
    (order_uuid, menu2_uuid, 'ชาไทย', 1, 25.00, 25.00);
END $$;
